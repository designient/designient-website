      ...
      contentClassName="py-16 md:py-24">
        <motion.div
          ...
          <div className="flex items-center justify-center gap-2 mb-6 text-sm" style={{ color: 'var(--text-muted)' }}>
            ...
            <ChevronRight className="w-4 h-4" aria-hidden="true" />
            <span style={{ color: 'var(--color-primary)' }} aria-current="page">{title}</span>
          </div>
          ...
          {badge && (
            <div className="mb-4 flex justify-center">
              <div
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full accent-icon-box"
                style={{ backgroundColor: 'var(--color-accent)' }}>
                <Award className="w-4 h-4" style={{ color: 'var(--text-on-accent)' }} aria-hidden="true" />
                <span className="font-body font-bold text-xs uppercase tracking-wide" style={{ color: 'var(--text-on-accent)' }}>
                  {badge}
                </span>
              </div>
            </div>
          ...
          <p
            ...
            {batchFull ? (
              <motion.div
                ...
                className="mb-8">
                <div className="bg-card rounded-xl p-4 md:p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex items-center justify-center gap-6 mb-8" role="group" aria-label="Course statistics">
            <Link href="/success-stories" className="flex items-center gap-2 group hover:opacity-80 transition-opacity">
              <div className="flex items-center gap-1" aria-label={`Rating: ${rating} out of 5 stars`}>
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-4 h-4 fill-current" style={{ color: 'var(--color-highlight)' }} aria-hidden="true" />
                ))}
              </div>
              <span className="font-body font-semibold text-sm group-hover:underline" style={{ color: 'var(--text-primary)' }} aria-label={`${rating} out of 5 rating`}>
                {rating}/5
              </span>
            </Link>
            <div className="h-4 w-px" style={{ backgroundColor: 'var(--border-default)' }} aria-hidden="true" />
            <Link href="/success-stories" className="font-body text-sm hover:underline transition-all" style={{ color: 'var(--text-muted)' }} aria-label={`${students} students enrolled`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 max-w-2xl mx-auto">
            {quickFacts.map((fact, index) => (
              <motion.div
                ...
                className="flex flex-col items-center gap-2 p-4 rounded-xl bg-card shadow-sm">
                <fact.icon className="w-6 h-6" style={{ color: 'var(--color-primary)' }} aria-hidden="true" />
                <span className="font-body text-xs font-medium text-center" style={{ color: 'var(--text-secondary)' }}>
                  {fact.label}
                </span>
              </motion.div>
            ))}
