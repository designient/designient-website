...
type CourseProofSectionInfographicProps = {
  section: PopularSectionData
  variant?: 'pro' | 'editorial'
  sectionId?: string
  badge?: string
  imageSrc?: string
  imageAlt?: string
...
export function CourseProofSectionInfographic({
  section,
  variant = 'editorial',
  sectionId = 'why-course',
  badge = 'Course highlight',
  imageSrc = '/images/class-photos/ui-ux-design-mentor-led-session-1.webp',
  imageAlt = 'mentor reviewing student work during live design session',
  overlayLabel = 'Designient track',
  footnote,
}: CourseProofSectionInfographicProps) {
  ...
  const popularStats =
    variant === 'pro'
      ? POPULAR_STATS.map((stat) =>
          stat.label === 'Average package' ? { ...stat, value: avgPackage } : stat
        )
      : []

  return (
    <section id={sectionId} className="py-16 md:py-24" style={{ backgroundColor: 'var(--bg-card)' }}>
      <div className="max-w-container mx-auto px-4 md:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
          ...
          >
            ...
            {(footnote ?? (variant === 'pro' ? PLACEMENT_STATS_FOOTNOTE : '')) && (
            <motion.p
              variants={fadeUp}
              initial={{ opacity: 0 }}
            ...
            >
              {footnote ?? PLACEMENT_STATS_FOOTNOTE}
            </motion.p>
            )}
          </motion.div>
        </div>
